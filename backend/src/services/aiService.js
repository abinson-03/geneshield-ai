...
You MUST respond with ONLY a valid JSON object (no markdown, no prose, no code fences). Format:
{
  ...
  "disclaimer": "This report is generated by GeneShield AI for educational purposes only. Consult a qualified healthcare professional or genetic counselor before making health decisions."
    ...
    // Confidence context for the AI — plain English, not jargon
    const confidenceLevel = variant.confidenceScore || 'Low';
    const confidenceContext = confidenceLevel === 'High'
      ? `Population confidence: HIGH — this variant has been studied in diverse populations including non-European ancestries, so the risk estimates are broadly applicable.`
      : confidenceLevel === 'Moderate'
      ? `Population confidence: MODERATE — this variant has been studied in a mixed group of populations. Results are likely meaningful for most people but may vary slightly by ancestry.`
      : `Population confidence: LOW — the research behind this finding comes mainly from studies of people of European ancestry. The risk estimate may be less precise for people from other backgrounds.`;
...
**IMPORTANT — how to handle population confidence:**
- If confidence is HIGH: state findings directly and clearly.
- If confidence is MODERATE: note briefly that results are broadly applicable but may vary slightly by ancestry.
- If confidence is LOW: you MUST include a plain-language caution somewhere in "yourRisk" or "whatItMeans" — for example: "It's worth noting that this finding is based mainly on European-ancestry data, so it may be less precise for people from other backgrounds." Do NOT bury this caution. Do NOT use clinical jargon. Keep it one sentence, conversational, and reassuring rather than alarming.
...
Respond with ONLY valid JSON (no markdown, no code fences):
{
  ...
  "whatItMeans": "Plain English explanation mentioning the specific gene function and why this variant matters (2-3 sentences). If confidence is LOW, include the ancestry caution here.",
  "yourRisk": "Specific risk based on genotype ${variant.userGenotype || 'unknown'} with percentage comparisons from literature if available (2-3 sentences). If confidence is LOW and it wasn't mentioned in whatItMeans, include the ancestry caution here instead.",
  ...
  if (apiKey && (apiKey.trim().startsWith('sk-') || apiKey.trim().startsWith('gsk_'))) {
    try {
      ...
      const variantDetails = variants.map(v => ({
        ...
        confidenceScore: v.confidenceScore || 'Low'
      }));

      const prompt = `You are GeneShield AI, a board-certified clinical genomicist and bioinformatician.

A user has uploaded their genomic sequencing file. Below are matched genetic variants, each with a "confidenceScore" field:
${JSON.stringify(variantDetails, null, 2)}

Overall Genomic Risk Score: ${overallRiskScore}/100

**IMPORTANT — population confidence:**
Each variant has a confidenceScore (High / Moderate / Low) reflecting how well-studied it is across diverse populations:
- High: studied in diverse groups including non-European ancestries — state findings clearly.
- Moderate: studied in a broad mix — findings are generally applicable, note briefly if relevant.
- Low: research comes mainly from European-ancestry studies — you MUST add a brief, plain-language note for each Low-confidence finding in its "keyAdvice", e.g. "Keep in mind this finding is based mainly on European-ancestry data and may be less precise for people from other backgrounds." Use one conversational sentence — not alarming, not clinical jargon.

If most variants are Low confidence, mention it once in the "overview" rather than repeating it per variant.
